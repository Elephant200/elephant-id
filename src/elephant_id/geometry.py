"""Generic planar-curve and envelope geometry.

Primitives shared by the feature finders (tears, holes) and the coding
pipeline. Nothing in here knows about elephants; callers supply curves and
radii. Design rationale: docs/tear-embedding.md.
"""
...
def opened_contour(P: np.ndarray, r: float) -> np.ndarray:
    """Open contour with outward spurs shaved off (morphological opening).
...
def alpha_shape(P: np.ndarray, radius: float) -> shapely.Polygon:
    ...
    fragment. (Implemented directly; the PyPI ``alphashape`` package has an
    edge-bookkeeping defect that collapses on this data.)
    """
