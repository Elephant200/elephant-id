...
- Levels: `debug` for cache hits/misses and fine detail; `info` for milestones (a model run finishing, a sighting coded); `warning` for recoverable or needs-review cases (log a `warning` before raising on a known failure path); `error`/`exception` for failures.
